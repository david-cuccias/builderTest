...
const createPagesAsync = async (config, createPage, graphql, models, offsets) => {
  const result = await graphql(`
    query {
      ${config.fieldName} {
        ${models.map((model, index) => `${model}(limit: ${config.limit}, offset: ${offsets[index]}, options: { cacheSeconds: 2, staleCacheSeconds: 2 }) {
            content
          }`).join(` `)}
      }
    }
...
exports.onCreatePage = ({
  ...
  if (page.path === '/dev-404-page/' && config.overrideDev404) {
    ...
    }, page.context); // override gatsby's custom 404 dev to allow development on Builder.io
